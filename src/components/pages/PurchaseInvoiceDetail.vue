<template>
  <f7-page name="PurchaseInvoiceDetail">
    <f7-navbar back-link="Back" sliding>
      <f7-nav-center>Purchase Invoice Detail</f7-nav-center>
    </f7-navbar>
    ...
    <section class="pz-width100 pz-size-normal pz-padding-t16" v-if="data">
      ...
      <div class="row pz-padding-tb-4 pz-padding-lr16 pz-bg-gray-lightest">
        ...
        <div
          class="col-65"
          v-if="!invoiceImages && $pzGlobalReactiveData.roleAccess('inventory', 'update')"
        >
          <image-uploader
            :maxCount="4"
        ...
        <div class="col-65" v-if="invoiceImages">
          <img
            v-for="(image, index) in invoiceImages"
            :key="index"
            :src="image"
            class="pz-margin-r8 image"
            @click="openZoomView(index)"
          >
        </div>
      </div>
    </section>
        ...
        <table class="equalDivide">
          ...
          <tbody>
            <tr v-for="(p, index) in data.stock" :key="p.id">
              ...
              <td
                class="numeric-cell"
              >{{(p.mrp - (p.mrp*p.procurement_discount/100))*p.original_quantity | moneyFormat}}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    getDetails() {
      window.vm.$http
        .get(`${window._pz.apiEndPt}purchase_invoice/${this.id}`)
        .then(res => {
    ...
    uploadImage(images) {
      ...
      window.vm.$http
        .patch(`${window._pz.apiEndPt}purchase_invoice?updateType=general`, {
          id: this.data.id,
          images: params
        })
        .then(res => {
