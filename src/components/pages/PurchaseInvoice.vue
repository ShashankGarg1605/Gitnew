<template>
  ...
  >
    <f7-navbar>
      ...
      <f7-nav-center>Purchase Invoice</f7-nav-center>
    </f7-navbar>
    ...
    <f7-list>
      <div v-if="allInvoice.length" class="list-block">
        <ul>
          <li
            ...
            :key="invoice.id"
            :class="{ 'pz-bg-red-lightest': !invoice.image }"
          >
            <div class="item-inner" style="flex-direction: column;">
              ...
              <div class="row pz-width100">
                ...
                <div
                  class="col-70"
                >{{invoice.publisher ? invoice.publisher.name : invoice.distributors.distributor_name}}</div>
              </div>
              <div class="row pz-width100">
              ...
              <i
                ...
                @click="openPopover(invoice.id, $event)"
              >more_horiz</i>
      ...
      <div
        ...
        v-if="allInvoice.length && hasReachedEnd && !$pzGlobalReactiveData.pendingReq "
      >Thats all folks!</div>
      <div
        class="color-gray"
        style="text-align: center; font-style: italic; "
        v-if="!allInvoice.length && !$pzGlobalReactiveData.pendingReq "
    ...
    <f7-popover id="pz-popover-2">
      <div class="popover-inner">
        <div class="list-block">
          <a
            @click="openPage( 'PurchaseInvoiceDetail')"
...
export default {
  ...
  computed: {
    filterQuery() {
      ...
      if (imageUploaded !== null)
        filterQuery += `&image_uploaded=${imageUploaded}`;
      ...
      if (invoiceNumber !== null)
        filterQuery += `&invoice_number=${invoiceNumber}`;
  ...
  methods: {
    getAllInvoices() {
      ...
      let url =
        `${window._pz.apiEndPt}purchase_invoice?limit=${this.limit}&offset=${
          this.offset
        }&orderBy=invoice_date&orderByValue=desc` + this.filterQuery;
      window.vm.$http
        .get(url)
        .then(res => {
