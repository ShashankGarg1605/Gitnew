<template>
  <f7-page name="PlaceImageOrder">
    <f7-navbar back-link="Back" sliding>
      <f7-nav-center>Place Image Order</f7-nav-center>
    </f7-navbar>
...
export default {
  ...
  methods: {
    uploadImage(images) {
      ...
      window.vm.$http
        .post(window._pz.apiEndPt + "orders/image?user=" + this.userID, params)
        .then(res => {
          window.vm.$f7.hidePreloader();
          if (res.ok) {
            window.vm.$f7.addNotification({
              message: `Image order ID ${res.body.id} placed!`,
              hold: 5000
            });
            let prevPage =
