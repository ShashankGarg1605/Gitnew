<template>
  ...
  >
    <f7-navbar v-bind="$pzGlobalReactiveData.navHistory.length>1?{ 'back-link': 'back' }:''">
      ...
      <f7-nav-center>All Image Orders</f7-nav-center>
    </f7-navbar>

    <f7-fab
      color="pink"
      href="/PlaceImageOrder"
      v-if="$pzGlobalReactiveData.roleAccess('imageorders', 'create')"
    >
          ...
          <li class="item-content" v-for="order in allOrders" :key="order.id">
            <div class="item-inner" style="flex-direction: column;">
              ...
              <div v-if="order.listOfImages && order.listOfImages.length" style="width:100%;">
                ...
                <span
                  v-for="(image, index) in order.listOfImages"
                  :key="index"
  ...
  methods: {
    getAllOrders() {
      let url =
        `${
          window._pz.apiEndPt
        }orders/images?orderBy=created_date&orderByValue=desc&limit=${
          this.limit
