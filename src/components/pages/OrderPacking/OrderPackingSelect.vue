            ...
            <li class="item-content" v-for="order in orders" :key="order.id">
              <div class="item-inner" style="flex-direction: column;">
                ...
                <div class="row pz-width100">
                  ...
                  <div
                    :class="`col-70 color-${order.packing_status}`"
                  >{{$pzGlobalReactiveData.packingStatusMap[order.packing_status]}}</div>
                </div>
              </div>
              <i
  ...
  methods: {
    ...
    startPacking() {
      window.vm.$http
        ...
        .then(res => {
          if (res.ok) this.gotToCartonSelect();
        })
        .catch(window._pz.errFunc2.bind(this));
    },
    gotToCartonSelect() {
      let url = `CartonSelect?orderId=${this.clickedOrder.id}`;
