  ...
  <section>
    ...
    <f7-page name="UserDetail">
      <section class="pz-width100 pz-size-normal pz-padding-t16" v-if="data">
        ...
        <list-item :label="'Status'" :value="$pzGlobalReactiveData.userStatusMap[data.status]"/>
        ...
        <list-item
          :label="'Business Type'"
          :value="$pzGlobalReactiveData.businessTypeMap[data.business_type]"
        />
        ...
        <list-item
          :label="'Logistics Borne By'"
          :value="$pzGlobalReactiveData.logisticStatusMap[data.logistic_status]"
        />
        <list-item
          :label="'Default Carrier'"
          :value="data.carrier_info && data.carrier_info.carrier.name"
        />
        ...
        <list-item
          :label="'Account Type'"
          :value="$pzGlobalReactiveData.accountTypeMap[data.account_type]"
        />
        <list-item
          :label="'Bilty Requirements'"
          :value="$pzGlobalReactiveData.biltyTypeMap[data.billt_requirement_code]"
        />
        <list-item :label="'Credit Period'" :value="data.credit_period + ' days'"/>
        <list-item :label="'Credit Limit'" :value="data.credit_limit | moneyFormat"/>
        <list-item :label="'Last App Use'" :value="data.last_app_use | dateFormat"/>
        ...
        <list-item :label="'PAN Image'" v-if="panImage">
          <img
            ...
            @click="$pzGlobalReactiveData.openZoomView(panImage)"
          >
        </list-item>
        <list-item :label="'CIN Image'" v-if="cinImage">
          <img
            :src="cinImage"
            class="pz-margin-r8 image"
            @click="$pzGlobalReactiveData.openZoomView(cinImage)"
          >
        </list-item>
        <list-item :label="'Shop License Image'" v-if="shopLicenseImage">
          <img
            :src="shopLicenseImage"
            class="pz-margin-r8 image"
            @click="$pzGlobalReactiveData.openZoomView(shopLicenseImage)"
          >
        </list-item>
      </section>
      ...
      <div
        ...
        v-if="!data && !$pzGlobalReactiveData.pendingReq"
      >{{errMsg}}</div>
    </f7-page>
  </section>
</template>
  ...
  computed: {
    ...
    shippingAddr() {
      if (!this.data) return null;
        ...
        const shipAddress = this.data.userAddress.find(
          _ => _.address_type === 2
        );
        const regAddress = this.data.userAddress.find(
          _ => _.address_type === 1
        );
    ...
    panImage() {
      if (!this.data || !this.data.pan_image) return null;
        ...
        return window._pz.uploadsEndPt + "userdocs/pan/" + this.data.pan_image;
    },
    cinImage() {
      if (!this.data || !this.data.cin_image) return null;
      else
        return window._pz.uploadsEndPt + "userdocs/cin/" + this.data.cin_image;
    },
    shopLicenseImage() {
      if (!this.data || !this.data.shop_licence_image) return null;
        ...
        return (
          ...
          "userdocs/shopLicense/" +
          this.data.shop_licence_image
        );
    }
  },
  methods: {
    getDetails() {
      window.vm.$http
        .get(`${window._pz.apiEndPt}users/${this.id}`)
        .then(res => {
          if (res.ok) {
            this.data = res.body;
          }
