...
export default {
  ...
  computed: {
    selCrrName() {
      if (!this.selectedCarrier) return null;
        ...
        return this.carriers.find(x => x.id === this.selectedCarrier).carrier
          .name;
    },
    biltyImage() {
    ...
    isPartiallyDispatched() {
      return (
        ...
        !this.data.orderStatus.some(
          el => el.status_id === 5 && el.bill_t_file_name
        ) &&
        !this.data.orderStatus.some(
    ...
    carrierName() {
      const idxStatus5 =
        ...
        this.data.orderStatus.findIndex(
          el => el.status_id === 5 && el.carrierTransportationDays
        );

      if (idxStatus5 !== -1)
        return this.data.orderStatus[idxStatus5].carrierTransportationDays
          .carrier.name;
      else return null;
    }
  },
  methods: {
    ...
    uploadImage(image) {
      ...
      window.vm.$http
        .patch(
          `${window._pz.apiEndPt}orders?id=${this.data
            .id}&modified_by=${localStorage.userID}&updateType=status&carrierId=${this
            .selectedCarrier}`,
          params
        )
        .then(res => {
    ...
    overrideDefaultBack(allOrdersFilters) {
      let prevPage =
        window.vm.$f7.mainView.history[window.vm.$f7.mainView.history.length - 2];
